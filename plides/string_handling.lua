...
function mprint(str, x, y, c, w)
  local function wrap_and_print(s, x, y, c, w)
    local max_chars = w and flr(w/4) or #s
    if #s <= max_chars then
      print(reverse_case(s), x, y, c)
      return y + 6
    end

    local word_index = max_chars
    while word_index > 0 and s[word_index] != " " do
      word_index -= 1
    end

    if word_index == 0 then word_index = max_chars end

    local allowed_str = sub(s, 1, word_index)
    local rest_str = sub(s, word_index+1)

    print(reverse_case(allowed_str), x, y, c)
    return wrap_and_print(rest_str, x, y+6, c, w)
  end
  ...
  for i=1,#str do
    if str[i] == "\n" then
      cur_y = wrap_and_print(cur_line, x, cur_y, c, w)
      cur_line = ""
    else
      cur_line ..= str[i]
    end

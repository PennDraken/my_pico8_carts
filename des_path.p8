...
function map_cost(from, node, graph)
    ...
    --return 1
    arr = {30,1,1,1,0.5,0.5,0.5,0.5,0.25,0.10,0.10}
    return arr[map_arr[node.x][node.y]+1]
end
...
function init_map()
    for i=1,128 do
        ...
        for j=1,128 do
            if rnd(1)>0.2 then
                map_arr[i][j] = 1 -- regular grass
            else
                map_arr[i][j] = 0 -- forest/ slow process
            end
        end
    end
end
...
function map_neighbors(pos, map_arr)
    ...
    if pos.x == nil then return end
    if pos.y == nil then return end
...
function draw_world(map_arr)
    for x = 1, 128 do
        for y = 1, 128 do
            ...
            if tile < grass then
                mpset(x, y, 2)
            elseif tile < dirt_path then
                mpset(x, y, 3)
            -- Dirt
            elseif tile < gravel_path then
                mpset(x, y, 4)
            -- Gravel
            elseif tile < stone_path then
                mpset(x, y, 6)
            -- Stone
            else
                mpset(x, y, 7)
...
-- How many needed to pass by to make it into a specific path
dirt_path = 3
gravel_path = 5
stone_path = 9
...
function _update()
    ...
    if stat(34) == 1 then
        if mousedown == false then
            add(house_arr, {x=stat(32) - 1,y=stat(33) - 1})
            add(npc_arr, new_npc(stat(32) - 1, stat(33) - 1))
            mousedown = true
        end
    else
        mousedown = false
...
function new_npc(x,y)
    ...
    npc.update = function(this)
        ...
        if rnd(map_arr[this.path[this.path_seg].x][this.path[this.path_seg].y]+5) < 4 then
            return
        end
        ...
        --use_tile(map_arr[this.x][this.y], x, y)
        if map_arr[this.x][this.y] < 10 then
            map_arr[this.x][this.y] += 1
        end
